...
course_code = re.compile(r"^[A-Z]{4}\s\d{3}:.*")
    ...
    with pdfplumber.open(file_p) as pdf:
        ...
        for page in pdf.pages[5:]:
            ...
            for line in lines:
                ...
                # Check for sub-headers (font size > 10 and <= 17) Classes need to be recognized as subheaders
                elif any(
                    10 < char["size"] <= 17 for char in line["chars"]
                ) or course_code.match(line_text):
                ...
                # Otherwise, it's normal text
                    ...
                    if current_header and current_subheader:
                        structured_text[current_header][current_subheader].append(
                            line_text
                        )
                    elif current_header:
